...
void main() {
  ...
  testWidgets('Home shows the design without gear or preview badge', (tester) async {
    ...
    expect(find.text('10 Tagen'), findsOneWidget);
    
    await tester.tap(find.text('Kalender öffnen ›'));
    await tester.pumpAndSettle();
    expect(find.text('Beispieldaten · September 2026'), findsOneWidget);
  });
  testWidgets('Calendar navigates across year boundary', (tester) async {
    await tester.pumpWidget(MaterialApp(home: Scaffold(body: CycleCalendar(today: DateTime(2026, 12, 18), periodDays: const {}))));
